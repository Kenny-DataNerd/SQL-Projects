...
---Next, i'll be adding a new column to the table to store the origin country data which will be used in my data visualization
ALTER TABLE netflix_dirty
...
/* The listed_in column has a combined data which will make my data visualization complicated
So i'll split the colums so i can access the genre and create a table to that effect */

---Creating a new column for genres
SELECT *,
       LEFT(listed_in, CHARINDEX(',', listed_in + ',') - 1) AS Genres
FROM netflix_dirty

---FInally, i'll be adding a new column to the table to store the Genre data which will be used in my data visualization

ALTER TABLE netflix_dirty
ADD Genres nvarchar(MAX);

UPDATE netflix_dirty
SET Genres = LEFT(listed_in, CHARINDEX(',', listed_in + ',') - 1);
...
---What are the top 10 Genres in Netflix?
...
---Who are the top 10 directors with the most contents on Netflix?
...
---What are the top 10 earliest released movies on Netflix?
...
---What are the top 10 oldest released movies on Netflix?
...
---Which production year has with highest contents on Netflix?
...
---What are the top content ratings 
